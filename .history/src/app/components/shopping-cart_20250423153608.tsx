...
export default function ShoppingCart(props: { cart: ICartItem[]; handlePayClick: () => void; setCart: Dispatch<SetStateAction<ICartItem[]>>; close: () => void;}){
    ...
    function handleTrashClick(e:any){
        if(e.target){
            ...
            const updatedOrder = shoppingCart?.filter(item => item.id !== productId);
           
            setShoppingCart( updatedOrder);
            props.setCart(updatedOrder)
        }
    }
                ...
                <>
                    ...
                    <div className="w-full flex h-fit justify-end">
                        <div className="w-full sm:w-1/2 md:w-1/4 h-50 p-5">
                            ...
                            <p className="text-center cursor gold pt-3 small" onClick={props.close}>Continue Shopping</p>
                        </div>
                    </div>
                </>    
                )
